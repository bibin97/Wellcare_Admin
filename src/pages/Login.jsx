    ...
    return (
        <form onSubmit={onSubmithandler} className='min-h-[80vh] flex items-center'>
            <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-gray-800 text-sm shadow-lg'>
                ...
                <button className='bg-blue-600 text-white w-full py-2 rounded-md text-base hover:bg-blue-700'>Login</button>
                {
                    state === 'Admin'
                        ? <p>Doctor Login? <span className='text-blue-500 underline cursor-pointer hover:text-blue-700' onClick={() => setState('Doctor')}>Click here</span></p>
                        : <p>Admin Login? <span className='text-blue-500 underline cursor-pointer hover:text-blue-700' onClick={() => setState('Admin')}>Click here</span></p>
                }
            </div>
        </form>
    )
